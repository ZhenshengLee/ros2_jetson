...
################################################################################
# 基于ga_team/ga_cuda镜像制作ga_all镜像
# cuda10.0+pcl1.9withcuda+opencv4.1withcuda+rosbase-melgaic
# 支持16.04 18.04
...
ARCH=$(uname -m)
# zs:
VERSION_3244="r32.4.4"
VERSION_3250="r32.5.0"
...
source ${GA_ROOT_DIR}/docker/scripts/ga_base.sh
check_agreement
...
# zs:
# 如果没有制定docker_repo，则默认是l4t-base
if [ -z "${DOCKER_REPO}" ]; then
    DOCKER_REPO=nvcr.io/nvidia/l4t-base
fi

# 如果没有制定,默认是3244
if [ -z "${GA_DIST}" ]; then
    GA_DIST="r32.4.4"
    VERSION=${VERSION_3244}
else
    GA_DIST="r32.5.0"
    VERSION=${VERSION_3250}
fi
...
function local_volumes() {
    ...
    volumes="-v $GA_ROS_ROOT_DIR:/ros2_jetson \
             -v $HOME/.cache:${DOCKER_HOME}/.cache\
             -v /root/.colcon:/root/.colcon\
             -v $HOME/.colcon:${DOCKER_HOME}/.colcon "
    case "$(uname -s)" in
        Linux)
...
function main(){
    ...
    # zs:
    GA_CONTAINER_NAME="zs_jetson_${GA_DIST}_${USER}"
    ...
    if [ "${USER}" != "root" ]; then
        # zs:
        docker exec $GA_CONTAINER_NAME bash -c '/ros2_jetson/docker/scripts/docker_adduser_jetson.sh'
    fi

    # zs:
    ok "Finished setting up environment. Now you can enter with: \nbash docker/scripts/docker_jetson_into.sh "
    ok "Enjoy!"
}
